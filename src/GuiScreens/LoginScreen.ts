...
export class LoginScreen {
    ...
    constructor(app: PIXI.Application, bgSprite: PIXI.Sprite) {
        ...
        xhrBgRequest.open("GET", "https://rh.konesproxy.kones.tech/service/https://osu.ppy.sh/api/v2/seasonal-backgrounds", false);
        xhrBgRequest.send();
        const bgJson = JSON.parse(xhrBgRequest.response).backgrounds;
        const bgImgUrl = "https://rh.konesproxy.kones.tech/service/"+bgJson[Math.round(Math.random() * ((bgJson.length-1) - 0) + 0)].url;
